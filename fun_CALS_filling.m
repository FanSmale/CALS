function [missRate,accuracy,totalCost,averCost,queryNumbers,buyLable]=fun_CALS_filling(data,dataOri,randomX,dc,ratio,distanceSelection,normalization,data_filling)
...
if normalization == 1
    ...
    X = X.*randomX;
    OriX = mapminmax(OriX);
    data_filling = mapminmax(data_filling);
end
...
% data_F = fillmissing(data,'makima');
% 
% data_filling = [ones(1,n);(data_F(:,1:end-1))'];
% % spling:分段三次样条插值; pchip:保形分段三次样条插值; makima:修正 Akima 三次 Hermite 插值
...
%                 tempIncomData(scheme(jj,:)) = averValue(scheme(jj,:));               %%将各种排列组合时，averValue中缺值位置的值赋给tempIncomData中相应位置
                tempIncomData(scheme(jj,:)) = data_filling(scheme(jj,:),i);               %%将各种排列组合时，averValue中缺值位置的值赋给tempIncomData中相应位置
                %%计算每一类的概率
                predProbability(:,i) = theta'*tempIncomData;                         %%计算z1，z2，z3
                predProbability(:,i) = exp(predProbability(:,i));                    %%计算e^z1，e^z2，e^z3
                sumProb(i) = sum(predProbability(:,i));                              %%计算e^z1+e^z2+e^z3
